...
//    TODO-Read查詢Product單筆資料
    ...
    public ResponseEntity<ApiResponse<Product>> getProductById(@PathVariable Long productsId) {
...
//    TODO-PUT-查詢編號並更新口味
    ...
    public ResponseEntity<ApiResponse<Product>> updateProduct(@PathVariable Long productsId,@RequestBody ProductRequest productRequest){
//        檢查productsId是否存在
            Product product = productService.getProductById(productsId);
            if(product==null){
//                return ResponseEntity.status(HttpStatus.NOT_FOUND).build();//404
...
//            return ResponseEntity.status(HttpStatus.OK).body(updaateProduct);//200
        return ResponseEntity.ok(new ApiResponse<>(true,updaateProduct,null));
    }
//    TODO-DELETE-刪除一筆資料
    @DeleteMapping("/products/{productsId}")
    public ResponseEntity<ApiResponse<Product>> deleteProduct(@PathVariable  Long productsId){
        Product product = productService.getProductById(productsId);
        if(product==null){
//            return ResponseEntity.status(HttpStatus.NOT_FOUND).build();//404
            return ResponseEntity
                    .status(HttpStatus.NOT_FOUND)
                    .body(new ApiResponse<>(false, null, "Product not found"));//204
        }
